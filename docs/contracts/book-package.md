...
- `book.json` at the ZIP root (not nested in a subfolder). [Intended — required]
- `chapters/chapter-N.html` for every `N` in `1..count`, where `count` is the chapter count declared in `book.json` / `BookMeta.chapterCount`. 1-based, no `chapter-0.html`. [Observed — `domain-model.md` Invariants, `business-rules.md`]
- Valid ZIP contains exactly that layout. App accepts only the exact archive-root layout and rejects any other shape (including outer-folder or `__MACOSX` wrappers). The producer must fix the ZIP; the app does not adapt to wrappers.
...
- `id` (string slug) determines the local folder `Application Support/novels/books/<id>/`, the Reader route `bookId`, and SQLite `book_id`. Remote numeric catalog ids do not affect it.
