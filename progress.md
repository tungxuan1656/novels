...
**Evidence**: `docs/plans/feat-004.md` (752 lines) + commits `5135ab1` TextSpan/HtmlParser 5 tests, `d068d72` ReaderViewModel 8 tests (fixture `test-slug` count 3), `fbf9815` References 3 tests, `d1eec46` BottomSheet 3 tests, `044c6d1` ReaderView 3 tests, `d50d6aa` Router `references(bookId:)` 4 tests, `8de01b5` Integration polish 2 tests + a11y prevButton/nextButton "Chương trước/sau" + offline `grep -R URLSession/http` 0 `grep -R WebKit/WKWebView` 0 — total `apps/novelsTests` ~70 tests + `novelsUITests` 2 — `./init.sh` PASS (format 0/58 lint 0 build PASS test PASS 165.7s Xcode 26.5 iPhone 17 Pro), `xcodebuild build` PASS, `xcodebuild test` PASS
**Blockers**: none
